...
const upload = multer({ 
  ...
  fileFilter: (req, file, cb) => {
    ...
    const videoTypes = ['.mp4', '.mkv', '.webm', '.mov', '.avi', '.mp3', '.wav', '.m4a'];
    const voiceTypes = ['.mp3', '.wav', '.m4a', '.aac', '.ogg', '.flac'];
    
    const ext = path.extname(file.originalname).toLowerCase();
    const allowedTypes = file.fieldname === 'voiceFile' ? voiceTypes : videoTypes;
    
    if (allowedTypes.includes(ext)) {
      cb(null, true);
    } else {
      cb(new Error(`File type ${ext} not allowed for ${file.fieldname}. Allowed: ${allowedTypes.join(', ')}`));
    }
  }
});
...
function buildCacheKey(params) {
  ...
  const hash = crypto.createHash('sha256').update(JSON.stringify(keyData)).digest('hex').slice(0, 16);
  return hash;
}
...
function getCachedResult(cacheKey) {
  ...
  try {
    ...
    const hasVideo = cached.result?.videoUrl && fs.existsSync(path.join(__dirname, cached.result.videoUrl.replace('/audio/', 'output/')));
    const hasAudio = cached.result?.audioUrl && fs.existsSync(path.join(__dirname, cached.result.audioUrl.replace('/audio/', 'output/')));
    ...
    console.log(`⚡ Cache HIT for ${cacheKey} (created ${new Date(cached.createdAt).toLocaleString()})`);
...
router.get('/options', (req, res) => {
  res.json({
    levels: [
      { value: 'A1', label: 'A1 - Superbeginner', desc: '500 words, present tense only' },
      { value: 'A2', label: 'A2 - Beginner', desc: '1500 words, simple past' },
      { value: 'B1', label: 'B1 - Intermediate', desc: '3000 words, all indicative' },
    ...
    modes: [
      { value: 'synced', label: 'Synced', desc: 'Direct translation, original timing', icon: '🎬' },
      { value: 'narrator', label: 'Narrator', desc: 'Time-filling, more words, slower speech', icon: '🎙️' },
      { value: 'narrator-only', label: 'Narrator Only', desc: 'Only dub the main speaker', icon: '🎤' },
      { value: 'learner', label: 'Learner', desc: 'Slower TTS (0.8x), audio-only', icon: '📚' },
      { value: 'extended', label: 'Extended', desc: 'Video stretched to fit full translation', icon: '⏱️' },
      { value: 'brainrot', label: 'Brainrot', desc: 'TikTok-style narration, sped-up video', icon: '🧠' },
    ],
    languages: [
      { value: 'spanish', label: 'Spanish', voices: ['male', 'female', 'neutral'] },
      { value: 'indonesian', label: 'Indonesian', voices: ['firman', 'bian', 'meraki'] },
    ],
    flags: [
      { value: 'premium', label: 'Premium TTS', desc: 'Use ElevenLabs (higher quality)', icon: '✨' },
      { value: 'clone', label: 'Voice Clone', desc: 'Clone original speaker voice (XTTS)', icon: '🎭' },
      { value: 'lipsync', label: 'Lip Sync', desc: 'AI lip-sync (Sync Labs)', icon: '👄' },
      { value: 'quality', label: 'High Quality', desc: 'Better audio separation (slower)', icon: '🎵' },
    ],
    narratorModes: [
      { 
        value: 'clone_speaker', 
    ...
    voiceSources: [
      { value: 'video', label: 'From Video', desc: 'Extract voice from input video' },
      { value: 'file', label: 'Upload File', desc: 'Upload your own voice sample (6-30s)' },
      { value: 'youtube', label: 'YouTube URL', desc: 'Extract voice from any YouTube video' },
    ],
  });
});
...
router.post('/process', upload.single('voiceFile'), async (req, res) => {
  ...
  const parsedNarratorOptions = typeof narratorOptions === 'string' ? JSON.parse(narratorOptions) : narratorOptions;
  const parsedTiktokOptions = typeof tiktokOptions === 'string' ? JSON.parse(tiktokOptions) : tiktokOptions;

  // Check cache for identical request
  const cacheKey = buildCacheKey({
    source: url, level, voice, mode, language,
    flags: parsedFlags, narratorOptions: parsedNarratorOptions, tiktokOptions: parsedTiktokOptions,
    startTime, duration, speakers, assignVoices,
  });
  ...
  if (cachedResult) {
    ...
    jobs.set(jobId, {
      ...
      options: { url, level, voice, mode, language, flags, narratorOptions, tiktokOptions, startTime, duration },
      createdAt: Date.now(),
...
router.post('/process-file', multiUpload, async (req, res) => {
  ...
  const parsedFlags = typeof flags === 'string' ? JSON.parse(flags) : flags;
  const parsedNarratorOptions = typeof narratorOptions === 'string' ? JSON.parse(narratorOptions) : narratorOptions;
  const parsedTiktokOptions = tiktokOptions && typeof tiktokOptions === 'string' ? JSON.parse(tiktokOptions) : tiktokOptions;
  ...
  const cacheKey = buildCacheKey({
    source: fileSource, level, voice, mode, language,
    flags: parsedFlags, narratorOptions: parsedNarratorOptions, tiktokOptions: parsedTiktokOptions,
    startTime, duration, speakers, assignVoices,
  });
  ...
  if (cachedResult) {
    ...
    console.log(`⚡ Returning cached result for ${mainFile.originalname} [${cacheKey}]`);
    return res.json({ jobId, status: 'completed', cached: true });
  }
...
router.delete('/cache', (req, res) => {
  ...
  if (url) {
    ...
    let cleared = 0;
    const files = fs.readdirSync(RESULT_CACHE_DIR).filter(f => f.endsWith('.json'));
    for (const file of files) {
      try {
        const cached = JSON.parse(fs.readFileSync(path.join(RESULT_CACHE_DIR, file), 'utf8'));
    ...
    res.json({ cleared, message: `Cleared ${cleared} cached results for this URL` });
  } else {
    // Clear all cache
    const files = fs.readdirSync(RESULT_CACHE_DIR).filter(f => f.endsWith('.json'));
    for (const file of files) {
    ...
    res.json({ cleared: files.length, message: `Cleared all ${files.length} cached results` });
  }
});
...
router.get('/cache', (req, res) => {
  ...
  const entries = files.map(f => {
    try {
      const cached = JSON.parse(fs.readFileSync(path.join(RESULT_CACHE_DIR, f), 'utf8'));
      return {
...
router.post('/extract-voice', upload.single('file'), async (req, res) => {
  ...
  let cacheKey;
  if (url) {
    cacheKey = crypto.createHash('md5').update(url + mode + samplesPerSpeaker).digest('hex').substring(0, 16);
  } else {
    const fileHash = crypto.createHash('md5').update(req.file.originalname + req.file.size).digest('hex').substring(0, 16);
  ...
  try {
    ...
    if (url) {
      ...
      if (fs.existsSync(cachedYtAudioPath)) {
        ...
        const { getCachedVideo, getCacheKey, detectSourceType } = require('./src/v2/ingest');
        ...
        if (cachedVideoPath) {
          ...
          try {
            ...
            // Use same options as ingest.js - android client bypasses 403 errors
            await youtubedl(url, {
    ...
    try {
      ...
      console.log(`   ⚠️ Spleeter failed, trying Demucs: ${err.message.substring(0, 80)}`);
      ...
      try {
        ...
        console.log(`   ⚠️ Both separation failed, using original: ${err2.message.substring(0, 50)}`);
      }
    }

    // AUTO MODE: Intelligent extraction
    if (mode === 'auto') {
      const { autoExtractVoiceSamples } = require('./src/v2/auto-voice-extract');
      ...
      const samples = result.allSamples.map((sample, idx) => {
        ...
        const permanentPath = path.join(permanentDir, path.basename(sample.path));
      ...
      result.speakers.forEach(speaker => {
        ...
        console.log(`      ${speaker}: ${speakerSamples.length} samples, best quality ${speakerSamples[0]?.qualityScore}/100`);
      });
    ...
    for (let i = 0; i < timePoints.length; i++) {
      ...
      const outputPath = path.join(extractDir, `sample_${i + 1}_${startTime}s.wav`);
      ...
      try {
        ...
        console.log(`   ⚠️ Sample ${i + 1} at ${startTime}s failed (may be past end of audio)`);
        continue;
      }
      ...
      const permanentPath = path.join(permanentDir, `sample_${i + 1}_${startTime}s.wav`);
      if (!fs.existsSync(permanentPath)) {
      ...
      console.log(`   ✅ Sample ${i + 1}: ${startTime}s (quality: ${analysis.qualityScore}/100)`);
    }

    if (samples.length === 0) {
      throw new Error('No samples could be extracted. Try different time points.');
    }
...
function runPipeline(jobId, options) {
  ...
  const args = [pipelinePath, options.source, options.level, options.voice, options.mode, options.language];
  ...
  if (options.characterTraits) args.push('--character-traits', options.characterTraits);
  
  // Add narrator options
  if (options.narratorOptions) {
    const { narratorMode, voiceSource, voiceStartTime, voiceDuration, customVoiceUrl, voiceFilePath } = options.narratorOptions;
  ...
  child.stdout.on('data', (data) => {
    ...
    else if (output.includes('SPLIT')) job.currentStep = 'Separating audio (Demucs)...';
    else if (output.includes('TRANSCRIBE')) job.currentStep = 'Transcribing speech...';
    else if (output.includes('TRANSLATE')) job.currentStep = 'AI translation...';
    else if (output.includes('TTS')) job.currentStep = 'Generating Spanish audio...';
    else if (output.includes('MERGE')) job.currentStep = 'Mixing audio...';
    else if (output.includes('RENDER')) job.currentStep = 'Rendering video...';
    else if (output.includes('LIP-SYNC')) job.currentStep = 'AI lip-sync (this takes a while)...';
    else if (output.includes('TIKTOK')) job.currentStep = 'Creating TikTok format...';
  ...
  child.on('close', async (code) => {
    if (code === 0) {
      ...
      if (jobDir) {
        ...
        const tiktokVideo = fs.existsSync(path.join(jobDir, 'dubbed_video_tiktok.mp4'))
        ...
        const voiceOnlyFile = fs.readdirSync(jobDir).find(f => f.includes('voice_only') && f.endsWith('.m4a'));
        const backgroundFile = fs.readdirSync(jobDir).find(f => f === 'background.mp3');
        ...
        job.result = {
          ...
          audioUrl: fs.existsSync(path.join(jobDir, 'dubbed_audio.mp3'))
            ...
            : (fs.existsSync(path.join(jobDir, 'dubbed_audio.m4a'))
              ...
              : null),
          voiceOnlyUrl: voiceOnlyFile ? `/audio/${dirBase}/${voiceOnlyFile}` : null,
          backgroundUrl: backgroundFile ? `/audio/${dirBase}/${backgroundFile}` : null,
      ...
      if (options.tiktokOptions && options.tiktokOptions.style && options.tiktokOptions.style !== 'none' && job.result?.videoUrl) {
        const inputVideo = path.join(__dirname, job.result.videoUrl.replace('/audio/', 'output/'));
        ...
        try {
          ...
          if (options.tiktokOptions.generateMetadata) {
            ...
            const { generateTikTokMetadata, extractVideoContext } = require('./src/v2/tiktok-hooks');
            
            // Try to extract context from transcript
            const transcriptPath = path.join(path.dirname(inputVideo), 'translation.json');
            let context = "Language learning video";
            
            if (fs.existsSync(transcriptPath)) {
              const transcript = JSON.parse(fs.readFileSync(transcriptPath, 'utf8'));
              const segments = transcript.segments || transcript;
              context = segments.slice(0, 3).map(s => s.originalText || s.text).join(" ");
            }
          ...
          const result = createTikTokFormat(inputVideo, outputVideo, options.tiktokOptions);
          
          if (result.success) {
            job.result.tiktokVideoUrl = job.result.videoUrl.replace('.mp4', '_tiktok.mp4');
            job.result.metadataUrl = metadata ? job.result.videoUrl.replace('.mp4', '_metadata.json') : null;
